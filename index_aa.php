...
function list_osa_scholarship($show_function=1){
	...
	";
	$data.="<div>
	<form id='form1' name='form1' method='post' action='index.php?op=sch_depart_menu'>物品編號查詢：$sch_depart_menu
...
function sch_depart_menu($sch_depart=''){
	global $xoopsDB;
	$sql = "select sch_depart from ".$xoopsDB->prefix("osa_scholarship")." where enable='1' group by sch_depart order by sch_depart";
	$result = $xoopsDB->query($sql) or redirect_header($_SERVER['PHP_SELF'],3, mysql_error());
	//die($sql);
...
$select="<select name='sch_depart' id='sch_depart'>
    ...
    $select.="<option value='$sch_depart'>$sch_depart</option>";
	}
$select.="</select>";
...
function select_category($category=''){
	...
	$sql = "select category from ".$xoopsDB->prefix("osa_scholarship")." where enable='1' group by category order by category";
	$result = $xoopsDB->query($sql) or redirect_header($_SERVER['PHP_SELF'],3, mysql_error());
	//die($sql);
		
$select="<select name='select' id='select' onchange=window.open(this.options[this.selectedIndex].value,'_self');>
		<option>請選擇分類</option>";
	while($all=$xoopsDB->fetchArray($result)){
    foreach($all as $k=>$v){
      $$k=$v;
	...
	if(!empty($category)){
    $select.="<option value='{$_SERVER['PHP_SELF']}?category=$url'>$category</option>";
	}
	}
$select.="</select>";
	return $select;
